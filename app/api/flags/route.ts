...
export async function POST(req: Request) {
    try {
        ...
        if (isGoodResult.length === 0) return NextResponse.json({ success: false, error: "Wrong flag !" }, { status: 403 });
        ...
        return NextResponse.json({ success: true, currentCoins: coins, currentPoint: points, challengeEnd: flags.length === countFlags.length }, { status: 200 })
    } catch (err) {
