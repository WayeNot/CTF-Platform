...
import { Status, statusBtn } from "@/lib/types"
...
export default function Home() {
    ...
    const { user_id, username, mail, bio, role, pp_url, status, updateUsername, updateMail, updateBio, updatePp_url, updateStatus } = useNavData()
    ...
    const updateProfile = async () => {
        await call(`/api/user/${user_id}/updateProfile`, { method: "PATCH", body: JSON.stringify({ username: username, mail: mail, bio: bio, pp_url: pp_url, status: status }) })
                ...
                <div className="flex flex-col gap-5 flex-1">
                    <div className="flex items-center gap-2">
                        <div className="flex flex-col gap-2 w-1/2">
                            <h2 className="font-bold text-white/60">Username</h2>
                            <input type="text" placeholder={"Your username"} className="p-2 border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 hover:text-white transition duration-500 active:scale-95" value={username} onChange={e => updateUsername(e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2 w-1/2">
                            <h2 className="font-bold text-white/60">Public mail</h2>
                            <input type="email" placeholder={"Your mail"} className="p-2 border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 hover:text-white transition duration-500 active:scale-95" value={mail} onChange={e => updateMail(e.target.value)} />
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="flex flex-col gap-2 w-1/2">
                            <h2 className="font-bold text-white/60">Display Picture</h2>
                            <input type="email" placeholder={"Your mail"} className="p-2 border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 hover:text-white transition duration-500 active:scale-95" value={pp_url} onChange={e => updatePp_url(e.target.value)} />
                        </div>
                        <div className="flex flex-col gap-2 w-1/2">
