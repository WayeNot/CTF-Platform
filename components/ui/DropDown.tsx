...
export default function DropDown(props: DropDownProps) {
    ...
    return (
        <div className="w-full relative h-full">
            <button onClick={onToggle} className={`w-full h-10.5 flex items-center justify-between gap-5 px-2 border text-xs transition duration-300 cursor-pointer ${selected.length ? "border-green-500/30 text-white" : "border-white/10 text-white/60"} hover:border-white/30 hover:text-white`}>
            ...
            {isOpen && (
                <div className="absolute left-0 top-full mt-2 w-full bg-[#363a3f] border border-white/10 p-2 flex flex-wrap gap-2 z-50 shadow-2xl max-h-52 overflow-y-auto">
                    {options.map(option => {
                        ...
                        return (
                            <button key={option.value} onClick={() => onSelect(option)} className={`px-2 py-1  text-xs border transition duration-300 cursor-pointer ${isActive ? "bg-green-500/10 text-green-300 border-green-500/30" : "bg-[#212529] text-white/50 border-transparent hover:brightness-150 hover:text-white"}`}>{option.label}</button>
                        );
                    })}
                </div>
            )}
